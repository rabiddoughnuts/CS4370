...
int calculateNumLevels(int size);
...
int main(){
    ...
    levelSizes[0] = ((Width + MAX_BLOCK_SIZE - 1) / MAX_BLOCK_SIZE);
    for(int i = 1; i < numLevels; i++) {
        levelSizes[i] = (levelSizes[i-1] + MAX_BLOCK_SIZE - 1) / MAX_BLOCK_SIZE;
    }
    
    for(int i = 0; i < numLevels; i++) {
        cudaMalloc(&d_levelSums[i], levelSizes[i] * sizeof(int));
    }
    ...
    ParPrefixKernel<<<dimGrid, dimBlock, shared_mem_size>>>(d_B, d_Sum, d_levelSums[0], Width);
    ...
    // Process higher levels if needed
    for(int level = 1; level < numLevels; level++) {
        dim3 levelGrid((levelSizes[level-1] + block_size - 1) / block_size);
        ParPrefixKernel<<<levelGrid, dimBlock, shared_mem_size>>>(
            d_levelSums[level-1], 
            d_levelSums[level], 
            nullptr,
            levelSizes[level-1]);
    ...
    // Add block sums back down through the levels
    for(int level = numLevels-1; level > 0; level--) {
        dim3 levelGrid((levelSizes[level-1] + block_size - 1) / block_size);
        AddScannedBlockSums<<<levelGrid, dimBlock>>>(
            d_levelSums[level-1],
            d_levelSums[level-1],
            d_levelSums[level],
            levelSizes[level-1]);
    ...
    // Clean up level arrays
    for(int i = 0; i < numLevels; i++) {
        cudaFree(d_levelSums[i]);
    }
    delete[] d_levelSums;
    delete[] levelSizes;
